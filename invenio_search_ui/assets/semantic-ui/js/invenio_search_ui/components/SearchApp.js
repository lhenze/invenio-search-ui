...
export const SearchApp = ({ config, appName }) => {
  ...
  const _buildUID = (element) => buildUID(element, "", appName);
  const context = {
    appName,
    buildUID: _buildUID,
    ...config,
  ...
  return (
    <OverridableContext.Provider value={overrideStore.getAll()}>
      <SearchConfigurationContext.Provider value={context}>
        ...
        >
          <Overridable id={_buildUID("SearchApp.layout")} config={config}>
            <Container>
              <Overridable id={_buildUID("SearchApp.searchbarContainer")}>
                <Grid relaxed padded>
                  <Grid.Row>
                    <Grid.Column width={4} />
                    <Grid.Column width={12}>
                      <Overridable id={_buildUID("SearchApp.searchbar")}>
                        <SearchBar />
                      </Overridable>
                    </Grid.Column>
                  </Grid.Row>
